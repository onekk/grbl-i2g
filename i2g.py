...
## i2g.py is free software; you can redistribute it and/or modify
...
import gettext;
gettext.install("i2g", localedir=os.path.join(BASE, "locale"), unicode=True)
...
def progress(a, b):
    print "FILTER_PROGRESS=%d" % int(a*100./b+.5)
...
def ui(im, nim, im_name):
    ...
    app = Tkinter.Tk()
    app.tk.call("source", os.path.join(BASE,"grbl-i2g" , "combobox.tcl"))
    ...
    def _optionmenu(k, v, *options):
        ...
        w = makewidget(f, Tkinter.Widget, wp)
        return w, var
...
def main():
    ...
    output = convert(nim, units, tool, pixel_size, step,
